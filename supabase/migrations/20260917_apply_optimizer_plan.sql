-- Atomic optimizer save; installing this migration does not move bookings.
-- The server-side portal client must use service_role. No public write RPC.
begin;

create table if not exists public.optimizer_plan_receipts (
    request_id uuid primary key,
    payload jsonb not null,
    result jsonb not null,
    saved_at timestamptz not null default now()
);
alter table public.optimizer_plan_receipts enable row level security;
revoke all on public.optimizer_plan_receipts from public, anon, authenticated;
grant select, insert on public.optimizer_plan_receipts to service_role;

create or replace function public.apply_optimizer_plan(
...
revoke all on function public.apply_optimizer_plan(uuid, integer, bigint, jsonb) from public, anon, authenticated;
grant execute on function public.apply_optimizer_plan(uuid, integer, bigint, jsonb) to service_role;
